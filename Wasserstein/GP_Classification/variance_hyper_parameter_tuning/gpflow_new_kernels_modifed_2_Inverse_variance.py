...
class ExpWassWithPsdApproximation(gpflow.kernels.Kernel):
    def __init__(self, cost_matrix: np.ndarray, xtrain: np.ndarray, xtest: np.ndarray,
                 inverse_variance= None, initial_scale=1.0, reg=0.4, stopThr=1e-9, numIterMax=1000,
                 precomputed_train_train_wassertein_distances = None,precomputed_train_test_wassertein_distances=None,
                 precomputed_test_test_wassertein_distances = None, y_train = None,y_test =  None,Maxiters_for_variance = 12):
        super().__init__()
        # self.variance = gpflow.Parameter(initial_variance, transform=positive())
        ...
        if inverse_variance is not None:
            self.inverse_variance = gpflow.Parameter(inverse_variance, transform=positive())
        else:
            print("Searching for Optimal variance...",'\n')
            self.inverse_variance = gpflow.Parameter(get_optimal_inverse_variance(self.train_train_wassertein_distances,self.test_train_wassertein_distances,
                                             self.test_test_wassertein_distances,xtrain,xtest,y_train,y_test,Maxiters_for_variance),transform=positive())
            print("Optimal inverse variance found",'\n')
    ...
    def K(self, X, X2=None):
        ...
        wassertein_distance_matrix = tf.constant(wassertein_distance_matrix, dtype=tf.dtypes.float64)
        exp_wassertein_non_psd_matrix = tf.exp(-tf.multiply(wassertein_distance_matrix,self.inverse_variance))
...
def get_optimal_inverse_variance(precomputed_train_train_wassertein_distances,precomputed_train_test_wassertein_distances,
                         precomputed_test_test_wassertein_distances,x_train,x_test,y_train,y_test,Maxiters):

    start = tf.constant(0.01,dtype=tf.float64)
    variance_upper_limit = tf.math.reduce_variance(precomputed_train_train_wassertein_distances) 
    inverse_variance_upper_limit = tf.divide(1,variance_upper_limit)
    print("inverse_variance_upper_limit: ",inverse_variance_upper_limit)
    end = tf.constant(6.0,dtype=tf.float64)
    ...
    model = gpflow.models.VGP(data=(x_train, y_train),
                                     ...
                                     kernel= ExpWassWithPsdApproximation(cost_matrix=m, inverse_variance=1.0,xtrain=x_train, xtest=x_test, initial_scale=1.0, reg=0.4, stopThr=5e-2,
    ...
    for var in tf.linspace(start,end,Maxiters):
        ...
        print('\n',"iteration: ",i)
        model.kernel.inverse_variance.assign(var)
        model.kernel.scale.assign(1.0)
        gpflow.utilities.print_summary(model)
        i += 1
