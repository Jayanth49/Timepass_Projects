...
class ExpWassWithPsdApproximation(gpflow.kernels.Kernel):
    def __init__(self, cost_matrix: np.ndarray, xtrain: np.ndarray, xtest: np.ndarray,
        ...
        if initial_variance is not None:
            ...
            self.variance = get_optimal_variance(precomputed_train_train_wassertein_distances,precomputed_train_test_wassertein_distances,
                                             precomputed_test_test_wassertein_distances,xtrain,xtest)
...
def get_optimal_variance(precomputed_train_train_wassertein_distances,precomputed_train_test_wassertein_distances,
                         precomputed_test_test_wassertein_distances,x_train,x_test,y_train):
    ...
    end = tf.constant(variance_upper_limit,dtype=tf.float64)
    ...
    variances = []
    i = 0
    for var in tf.linspace(start,end,5):
        print('\n',"iteration: ",i, '\n')
        i += 1
        variances.append(var)
        model = gpflow.models.VGP(data=(x_train, y_train),
                                      mean_function=gpflow.mean_functions.Zero(),
                                      kernel= ExpWassWithPsdApproximation(cost_matrix=m, initial_variance=var,xtrain=x_train, xtest=x_test, initial_scale=1.0, reg=0.4, stopThr=5e-2,
