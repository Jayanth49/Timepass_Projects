...
num_training_per_class = [1]#, 10, 20, 30, 40, 50]
all_accuracies = []
...
for num_train in num_training_per_class:
    ...
    # print('Computing Optimal variance')
    # optimal_variance = gpflow_new_kernels.get_optimal_variance(precomputed_train_train_wassertein_distances,precomputed_train_test_wassertein_distances,
    #                                          precomputed_test_test_wassertein_distances,x_train,x_test,y_train)
    
    C = 10
    model = gpflow.models.VGP(data=(x_train, y_train),
                              mean_function=gpflow.mean_functions.Zero(),
                              kernel=gpflow_new_kernels_modified_1.ExpWassWithPsdApproximation(
                                  cost_matrix=m,initial_variance= None, initial_scale=1.0, reg=0.4, stopThr=5e-2,
